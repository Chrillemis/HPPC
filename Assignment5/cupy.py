...
# Precision control
PREC = 4  # 4 for float32, 8 for float64
...
def rescale_grid(scale, H, lon, lat, dx, dy):
    ...
    new_lon = lon_min + 0.5*new_dlon + cp.arange(new_nx) * new_dlon
    new_lat = lat_min + 0.5*new_dlat + cp.arange(new_ny) * new_dlat
    ...
    x0 = cp.floor(x_new).astype(int)
    y0 = cp.floor(y_new).astype(int)

    # get the next indices for interpolation. In range [0, n], but clamp to n-1
    x1 = cp.minimum(x0 + 1, old_nx - 1)
    y1 = cp.minimum(y0 + 1, old_ny - 1)
    ...
    xf = cp.clip(x_new - x0, 0, 1)
    yf = cp.clip(y_new - y0, 0, 1)
    
    # Bilinear interpolation
    new_H = cp.empty((new_ny, new_nx), dtype=cp.float64)
...
def step(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, \
    ...
    eta_padded = cp.pad(eta, 1, mode='edge') # Pad with edge values for simple Neumann boundary conditions (zero gradient)
    laplacian_eta = (
    ...
    diffusion = 0.01 * cp.sqrt(g * H_at_u[:, :-1]) * min(dx, dy)
    ...
    u[:, 1:-1] = cp.where(wall_u[:, 1:-1], 0.0, u[:, 1:-1])
    u[:, 0] = u[:, 1]
    u[:, -1] = u[:, -2]
    ...
    v[1:-1, :] = cp.where(wall_v[1:-1, :], 0.0, v[1:-1, :])
    v[0, :] = v[1, :]
    v[-1, :] = v[-2, :]

# @cp.fuse()
# def step1(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, \
#          dx, dy, dt, friction, nx, ny):
#     # Step 1: ∂η/∂t = -∂(Hu)/∂x - ∂(Hv)/∂y (continuity equation)
#     dflux = (H_at_u[:, 1:] * u[:, 1:] - H_at_u[:, :-1] * u[:, :-1]) / dx + \
#             (H_at_v[1:, :] * v[1:, :] - H_at_v[:-1, :] * v[:-1, :]) / dy
...
# @cp.fuse()
# def step2(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, \
#          dx, dy, dt, friction, nx, ny):
#     # Step 2: ∂u/∂t = +fv - g·∂η/∂x - r·u
#     #         ∂v/∂t = -fu - g·∂η/∂y - r·v
...
# def step(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, \
#          dx, dy, dt, friction, nx, ny):
#     step1(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, dx, dy, dt, friction, nx, ny)
#     step2(eta, u, v, g, f_at_u, f_at_v, H_at_u, H_at_v, land_mask, wall_u, wall_v, dx, dy, dt, friction, nx, ny)
...
class ShallowWaterSolver:
    ...
    def __init__(self, elevation, lon, lat, dx, dy, cfl=0.5, friction=1e-5):
        ...
        self.H = cp.maximum(-elevation, 0.0) # Water depth (positive below sea level, oppposite sign of elevation)
        ...
        self.land_mask = cp.ascontiguousarray(self.H < 1., dtype=bool) # Land mask (where depth is very small)

        # Precompute H at u and v faces
        self.H_at_u = cp.zeros((self.ny, self.nx + 1), dtype=real_t)
        self.H_at_u[:, 1:-1] = 0.5 * (self.H[:, :-1] +  self.H[:, 1:])
        self.H_at_u[:, 0] = self.H[:, 0]
        self.H_at_u[:, -1] = self.H[:, -1]
        
        self.H_at_v = cp.zeros((self.ny + 1, self.nx), dtype=real_t)
        ...
        self.wall_u = cp.zeros((self.ny, self.nx + 1), dtype=bool)
        self.wall_u[:, 1:-1] = self.land_mask[:, :-1] | self.land_mask[:, 1:]
        
        self.wall_v = cp.zeros((self.ny + 1, self.nx), dtype=bool)
        ...
        self.f = (2 * omega * cp.sin(cp.deg2rad(lat))).astype(real_t)   # Coriolis parameter at cell centers (ny,)
        self.f_at_u = self.f[:, cp.newaxis] * cp.ones((1, self.nx + 1)) # Coriolis parameter at u faces (same as cell centers, since f varies only in y)
        self.f_at_v = cp.zeros((self.ny + 1, self.nx), dtype=real_t)    # Coriolis parameter at v faces (average of adjacent cell centers in y direction)
        self.f_at_v[1:-1, :] = 0.5 * (self.f[:-1, cp.newaxis] + self.f[1:, cp.newaxis])
        self.f_at_v[0, :] = self.f[0]
        self.f_at_v[-1, :] = self.f[-1]
        
        # Initialize state variables
        self.eta = cp.zeros((self.ny, self.nx), dtype=real_t)   # Surface elevation at cell centers
        self.u = cp.zeros((self.ny, self.nx + 1), dtype=real_t) # x-velocity at u faces
        self.v = cp.zeros((self.ny + 1, self.nx), dtype=real_t) # y-velocity at v faces
        
        # Calculate stable time step
        max_depth = cp.max(self.H)
        c_max = cp.sqrt(self.g * max_depth)
        ...
        print(f"  Land cells: {cp.sum(self.land_mask)} ({100*cp.sum(self.land_mask)/self.land_mask.size:.1f}%)")
    ...
    def set_gaussian_initial_condition(self, i_center, j_center, amplitude=20.0, spread=20):
        ...
        X, Y = cp.meshgrid(cp.arange(self.nx), cp.arange(self.ny))
        r2 = ((X - j_center) / spread)**2 + ((Y - i_center) / spread)**2
        self.eta = (amplitude * cp.exp(-0.5 * r2)).astype(real_t)
    ...
    def run(self, duration, output_interval=None):
        ...
        snapshot_event = cp.cuda.Event()
        
        eta_snapshot_gpu = cp.empty_like(self.eta)
        pinned = cp.cuda.alloc_pinned_memory(self.eta.nbytes)
        eta_snapshot_cpu = np.frombuffer(
            pinned,
            dtype=self.eta.dtype,
            count=self.eta.size
        ).reshape(self.eta.shape)        
        tlast = perf_counter()
        nupdated = 0
        for n in range(n_steps):
            with compute_stream:
                ...
                # print(cp.max(cp.abs(self.u)).get())

                nupdated += self.nx * self.ny # increment number of updated grid points (for performance measurement)
                if n % output_every == 0:
                    eta_snapshot_gpu[...] = self.eta
                    snapshot_event.record()
            if n % output_every == 0:
                transfer_stream.wait_event(snapshot_event)
                with transfer_stream:
                    eta_snapshot_gpu.get(out=eta_snapshot_cpu)
                    etas.append(eta_snapshot_cpu.copy())
                    ...
                    if n % (10 * output_every) == 0:
                        max_eta = np.max(np.abs(eta_snapshot_cpu))
...
def simulate(config):
    ...
    with open(input_file, 'rb') as f:
        ...
        lon = cp.frombuffer(f.read(8*nx), dtype=cp.float64)
        lat = cp.frombuffer(f.read(8*ny), dtype=cp.float64)
        elevation = cp.frombuffer(f.read(8*nx*ny), dtype=cp.float64).reshape((ny, nx))
    ...
    dlon = cp.abs(lon[1] - lon[0])
    dlat = cp.abs(lat[1] - lat[0])
    ...
    j_krakatau = cp.argmin(cp.abs(lon - krakatau_lon))
    i_krakatau = cp.argmin(cp.abs(lat - krakatau_lat))
    ...
    print("\nAverage performance is {:.2f} ns/grid point update".format(cp.mean(results['performance'][1:])))
    ...
    with open(config.filename, 'wb') as f:
        f.write(cp.array(nframes, dtype=cp.int32).tobytes())
        f.write(cp.array(nx, dtype=cp.int32).tobytes())
        f.write(cp.array(ny, dtype=cp.int32).tobytes())
        f.write(results['lon'].astype(cp.float32).tobytes())
        f.write(results['lat'].astype(cp.float32).tobytes())
        f.write(cp.array(results['time'], dtype=cp.float32).tobytes())
        f.write(cp.array(results['performance'], dtype=cp.float32).tobytes())
        f.write(cp.array(elevation, dtype=cp.float32).tobytes())
        for i in range(nframes):
            f.write(results['eta'][i].astype(cp.float32).tobytes())
